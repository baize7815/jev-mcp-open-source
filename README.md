...
自部署的 MCP 网关，运行在 Cloudflare Workers 上，把 TypeSafe / Jev 的 System One 模型包装成九个工具，供 AI 助手在本地完成**意图路由、检索结果重排、批量语义判断**这类"判断但不生成长文"的工作。配套提供一个 Codex Skill，让助手在合适的决策点主动调用它。
...
部署后 `/mcp` 暴露以下九个工具。所有判断都只返回结构化答案与置信度，不生成散文；服务端对每批请求做大小校验、并发限制和失败隔离。
...
| `select_values` | 从已知候选中选择字段值 | 实体消歧、结构化提取与参数填充 |
| `verify_evidence` | 判断所给证据支持、反驳或不足以判断声明 | RAG、研究与需求核验，不代替外部检索 |
| `evaluate_options` | 按明确标准评分，并用代码加权排序 | 多方案比较，保留原始评分供调整权重 |
| `classify_hierarchy` | 逐层选择分类，在不确定时停止 | 大类目与树状分类 |
| `decide_next_step` | 按当前目标和观察建议下一步 | 返回输入指纹辅助缓存失效判断，不执行动作 |
| `system_one` | 直通 TypeSafe System One，自定义 state 与问题 | 专用工具覆盖不了的自定义布尔/选项/打分判断 |

批量记录类工具限制：单批最多 **10** 条记录，总输入 **128 KB**，服务端并发 **3**。批量判断中失败的记录标记为 `failed`，不会被当作"无关"丢弃；不自动重试，避免重复计费。

## 0.3.0 与验证

本版提供 9 个工具，保留原有数据库与接口，无需新增数据库迁移。客户端重新连接后刷新工具列表。新增工作流的完整输入样例见 `eval/cases.json`，包括成功和不确定路径。

`npm run eval:check` 只检查 13 个合成评估用例的格式；`npm run eval:live` 必须显式配置 `JEV_MCP_URL`，会使用该地址的真实 `system_one` 执行评估并消耗额度，输出精确匹配结果和置信度阈值统计。它不是领域准确率保证。
...
专用工具之外的自定义判断，直接传 `state` 和 `questions`：
...
- 批量记录类工具每批最多 10 条，总输入 128 KB，并发 3；字段提取最多 20 个字段。其他限制以各工具的输入 schema 为准。
